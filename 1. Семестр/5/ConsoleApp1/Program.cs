...
namespace Lab3
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            H = H.Append(DenseMatrix.OfArray(c));

            double[,] lastRow = new double[1, H.ColumnCount];

            var rowMatrix = H.ColumnSums().Add(1);
            for (int y = 0; y < rowMatrix.Count; y++)
            {
                if (rowMatrix[y] % 2 == 0)
                {
                    rowMatrix[y] = 0;
            ...
            H = H.InsertRow(H.RowCount, rowMatrix);
            Console.WriteLine(H);

            var bar = new BitArray(bytes);
            var xr = new double[r + 1];
            ...
            Xr = Matrix<double>.Build.Dense(1, r + 1, (ii, jj) => (double)xr[jj]);
            ...
            if (S.Column(0).Sum() % 2 == 0)
            {
                Console.WriteLine("Вес четный - 2 ошибки");
            }
            else
            {
                Console.WriteLine("Вес нечетный - 1 ошибка");
            }
